...
assert_product() {
  ...
  if [[ "$(getprop ro.build.product)" == "$target_product" ||
  "$(getprop ro.product.device)" == "$target_product" ]]; then
    return 0
  else
    return 11
  fi
...
get_file_name 1 >/dev/null || abort "检查文件名错误，your file name is not correct"
...
run_part_tool() {
  ...
  if [[ "$(get_userdata_end)" == "${userdata_old_end}" ]]; then
    ui_print "调整分区失败，尝试备选方案..."
    ui_print "Adjusting partition failed,try alternative solution..."
  ...
  ${TOOLPATH}parted -s ${DISKPATH} mkpart esp ${esp_info[1]} ${esp_info[2]}GB ${esp_info[3]}GB
  sleep 1
  ui_print "esp分区创建成功,esp partition created"
  sleep 1
  ui_print "正在创建win分区..."
  ui_print "Creating win partition..."
  ${TOOLPATH}parted -s ${DISKPATH} mkpart win ${win_info[1]} ${win_info[2]}GB ${win_info[3]}GB
  sleep 1
  ui_print "win分区创建成功,win partition created"
  ${TOOLPATH}parted -s ${DISKPATH} set ${esp_info[0]} esp on
}
...
install_win() {
  ...
  ui_print "正在安装Windows，Installing Windows"
  ui_print "过程预计2-8分钟，请耐心等待"
  ui_print "The process is expected to take 2-8 minutes, please wait"
  ${TOOLPATH}wimlib-imagex apply --quiet $windata /dev/block/by-name/win || return 42
  ...
  tar -qf ${DATAPATH}efi.tar -C /tmp/esp/
  bcdboot /tmp/esp/EFI/Microsoft/Boot/BCD /dev/block/by-name/win || return 43
  umount /tmp/esp
  sleep 1
...
bak_img() {
  ...
  rm -rf /tmp/esp/
  mkdir -p /tmp/esp/
  mount /dev/block/by-name/esp /tmp/esp/ || return 50 || abort "esp分区挂载失败，esp partition mount failed"
  dd if=/dev/block/by-name/${partname} of=/tmp/esp/${filenaem} || local aaa=false
  umount -A /dev/block/by-name/esp
  rm -rf /tmp/esp/
  $aaa || return 51
}
...
bak_android_boot() {
  nowboot=$(get_ab)
  bak "boot${nowboot}" "android_boot.img" || return $?
}
...
flash_img() {
  ...
  dd if=/tmp/FWT/${filenaem} of=/dev/block/by-name/${partname} || local aaa=false
  $aaa || return 52
}
