...
# 恢复安卓boot
re_android_boot() {
  # 此函数因为逻辑问题未实际调用，具体方法参照reset.sh
  # 获取当前boot版本
  nowboot=$(get_ab)
  ui_print "- 正在恢复安卓boot..."
  ui_print "- Restoring Android_boot..."
  ${TOOLPATH}umount -A /dev/block/by-name/esp
  rm -rf /tmp/esp/
  mkdir -p /tmp/esp/
  mount /dev/block/by-name/esp /tmp/esp/ || return 50

  cp -rf /tmp/esp/android_boot.img ${DATAPATH} && flash_img android_boot.img boot${now_boot} || abort "恢复安卓boot失败，restore android_boot failed"
...
re_part() {
  ...
  re_run_sgdisk() {
    ...
    ${TOOLPATH}sgdisk ${DISKPATH} -l=${sgdisk_bak_file} || return 79
  }
